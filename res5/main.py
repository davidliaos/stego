    ...
    for idx, image_file in enumerate(images_to_encrypt, start=1):
        ...
        for i in range(min(img1.shape[0], img2.shape[0])):
            for j in range(min(img1.shape[1], img2.shape[1])):
                for l in range(3):
                    # Embed the 3 MSBs of img2 into the 3 LSBs of img1
                    img1[i][j][l] = (img1[i][j][l] & 0b11111000) | (img2[i][j][l] >> 5)
                    # Embed an additional bit from img2 into the 4th LSB of img1
                    img1[i][j][l] = (img1[i][j][l] & 0b11101111) | ((img2[i][j][l] & 0b00010000) >> 1)
...
def decrypt_lsb(encrypted_image, decrypted_folder, file_id):
    ...
    for i in range(width):
        for j in range(height):
            for l in range(3):
                # Retrieve the 3 LSB and the 4th LSB from img and reconstruct the 4 bits
                decrypted_img[i][j][l] = ((img[i][j][l] & 0b111) << 5) | ((img[i][j][l] & 0b00010000) << 1)

    decrypted_image_name = f'decrypted_{file_id}.png'
    cv2.imwrite(os.path.join(decrypted_folder, decrypted_image_name), decrypted_img)
