...
def encrypt_lsb_balanced(original_image, images_to_encrypt, dataset_folder, encrypted_folder):
    ...
    for idx, image_file in enumerate(images_to_encrypt, start=1):
        ...
        for i in range(img2.shape[0]):
            for j in range(img2.shape[1]):
                for l in range(3):
                    # Replace the 2 LSB of img1 with the 2 MSB of img2
                    img1[i][j][l] = (img1[i][j][l] & 0b11111100) | (img2[i][j][l] >> 6)

        encrypted_image_path = os.path.join(encrypted_folder, f'encrypted_{idx}.png')
        cv2.imwrite(encrypted_image_path, img1)

def decrypt_lsb_balanced(encrypted_image, decrypted_folder, file_id):
    ...
    for i in range(width):
        for j in range(height):
            for l in range(3):
                # Retrieve the 2 LSB and shift them to the 2 MSB position
                decrypted_img[i][j][l] = (img[i][j][l] & 0b11) << 6
...
def autodecrypt(encrypted_folder, decrypted_folder):
    ...
    for file_id, file_name in enumerate(encrypted_files, start=1):
        ...
        decrypt_lsb_balanced(encrypted_image_path, decrypted_folder, file_id)
...
encrypt_lsb_balanced(original_image, images_to_encrypt, dataset_folder, encrypted_folder)
